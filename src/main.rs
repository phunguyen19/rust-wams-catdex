...
fn app() -> Html {
    ...
    let styesheet = Style::new(CSS).unwrap();
    ...
    html! {
        <div class={styesheet}>
            ...
            <section class="cats">
                { for cat_list.iter().map(|v| cat(v, delete_cat.clone())) }
            </section>
        </div>
    }
}
...
struct CatDetails {
    name: String,
    image: Vec<u8>, // To store buffer from browser
}
...
fn cat(cat: &CatDetails, callback: Callback<String>) -> Html {
    html! {
        <article class="cat">
            ...
            <img src={
                format!("data:image;base64,{}", general_purpose::STANDARD.encode(&cat.image))
            } />
        </article>
    }
}
...
fn delete_button(button: &ButtonProp) -> Html {
    ...
    html! {
        <div>
            <button onclick={on_click}>{{button.text.clone()}}</button>
        </div>
    }
}
